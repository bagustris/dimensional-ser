# lstm_cnn: emotion recognition from speech= lstm, text=cnn
# created for ATSIT paper 2020
# coded by Bagus Tris Atmaja (bagus@ep.its.ac.id)
# chagnelog:
# 2020-01-12: Modified text to CNN
...
def model(alpha, beta, gamma):
    ...
    net_text = Dense(256, activation='relu')(net_text)
    net_text = Dense(256, activation='relu')(net_text)
    net_text = Dense(256, activation='relu')(net_text)
...
earlystop = EarlyStopping(monitor= 'val_loss', mode='max', patience=10,
...
np.save('../../data/predict_lstm_dense.npy', np.array(predict).reshape(3, 3743).T)
