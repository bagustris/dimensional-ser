...
if __name__ == '__main__':
    ...
    for alpha in loss_weight_step:
        for beta in loss_weight_step:
            for gamma in loss_weight_step:
                best = main(alpha, beta, gamma)
                weight.append([alpha, beta, gamma, best[4:], np.mean(best[4:])])
    weight_df = pd.DataFrame(weight)
    weight_df.to_csv('weight_optim_3.csv', header=['alpha', 'beta', 'gamma',
                     'best', 'ave'])
