...
# API model
def create_model(dropout_rate1, dropout_rate2):
    # speech network
    ...
    model_speech = Dropout(dropout_rate1)(net_speech)
    ...
    model_text = Dropout(dropout_rate2)(net_text)
...
def main(dropout1, dropout2):
    model = create_model(dropout1, dropout2)
    model.summary()
...
if __name__ == '__main__':
    ...
    for i in dropout_rates:
        for j in dropout_rates:
            print("Using dropout= ", i, j)
            best = main(i, j)
            score.append([i, j, best[4:], np.mean(best[4:])])
    weight_df = pd.DataFrame(score)
    weight_df.to_csv('weight_optim_ter.csv', header=['do_1', 'do_2', 'vad', 'ave'])
