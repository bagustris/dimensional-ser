...
def create_model(dropout_rate):
    ...
    model_text = Dropout(dropout_rate)(net_text)
...
if __name__ == '__main__':
    dropout_rates = np.arange(0.0, 1.1, 0.1)
    score = []
    for i in dropout_rates:
        print("Using dropout= ", i)
        best = main(i)
        score.append([i, best[4:], np.mean(best[4:])])
    weight_df = pd.DataFrame(score)
    weight_df.to_csv('weight_optim_ter.csv', header=['do_rate', 'vad', 'ave'])
